  ...
  public static class ConfigParser
  {
    ...
    internal static object ParseValue(Type type, string json)
    {
      if (type == typeof(string))
      {
        ...
        for (int i = 1; i < json.Length - 1; ++i)
        {
          if (json[i] == '\\' && i + 1 < json.Length - 1)
          {
            ...
            {
              if (UInt32.TryParse(json.Substring(i + 2, 4),
                System.Globalization.NumberStyles.AllowHexSpecifier, null, out uint c))
              {
      ...
      if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Dictionary<,>))
      {
        ...
        IDictionary dictionary = (IDictionary)type.GetConstructor(new Type[] { typeof(int) })
      ...
      for (int i = 0; i < members.Length; i++)
      {
        ...
        if (member.IsDefined(typeof(DataMemberAttribute), true))
        {
          DataMemberAttribute dataMemberAttribute = (DataMemberAttribute)
            Attribute.GetCustomAttribute(member, typeof(DataMemberAttribute), true);
    ...
    static object ParseObject(Type type, string json)
    {
      ...
      BindingFlags bindFlags = BindingFlags.Public | BindingFlags.Instance | BindingFlags.FlattenHierarchy;
      if (!fieldInfoCache.TryGetValue(type, out Dictionary<string, FieldInfo> nameToField))
      {
        nameToField = CreateMemberNameDictionary(type.GetFields(bindFlags));
        fieldInfoCache.Add(type, nameToField);
      }
      if (!propertyInfoCache.TryGetValue(type, out Dictionary<string, PropertyInfo> nameToProperty))
      {
        nameToProperty = CreateMemberNameDictionary(type.GetProperties(bindFlags));
